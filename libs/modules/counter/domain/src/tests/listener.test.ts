...
import { counterActions, counterSelectors, counterSlice } from '../counterService'
...
describe('counter - listeners', () => {
  ...
  beforeEach(() => {
    ...
    setupCounterListeners(listenerMiddlewareInstance.startListening as AppStartListening)
  })
  ...
  describe('onUpdateAsync', () => {
    ...
    it('asynchronously adds `payload.delta` after `payload.delayMs` to counter', async () => {
      ...
      expect(counterSelectors.selectById(store.getState(), id)?.value).toBe(initialValue)

      await delay(delayMs)

      expect(counterSelectors.selectById(store.getState(), id)?.value).toBe(initialValue + delta)
    })
  ...
  describe('onUpdateByPeriodically', () => {
    ...
    it('periodically adds `payload.delta` after `payload.intervalMs` to counter', async () => {
      ...
      store.dispatch(counterActions.updateByPeriodically({ id, intervalMs, delta }))

      for (let i = 0; i < 2; i++) {
        expect(counterSelectors.selectById(store.getState(), id)?.value).toBe(initialValue + i * delta)

        await delay(intervalMs)

        expect(counterSelectors.selectById(store.getState(), id)?.value).toBe(initialValue + (i + 1) * delta)
      }
    })
