...
function shouldStopAsyncTasksOf(id: string) {
  return isAllOf(
    ...
    (action: UnknownAction): action is PayloadAction<string> => action?.payload === id
  )
}

async function onUpdateByPeriodically(
  { payload: { id, delta } }: ReturnType<typeof counterActions.updateByPeriodically>,
...
async function onUpdateAsync(
  { payload: { id, delta, delayMs } }: ReturnType<typeof counterActions.updateByAsync>,
  { condition, dispatch, getState }: AppListenerEffectAPI
) {
  const counter = counterSelectors.selectById(getState(), id)
  console.log("🚀 ~ file: listeners.ts:45 ~ counter:", counter)
