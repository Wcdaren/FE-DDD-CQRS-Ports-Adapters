...
export const counterSlice = createSlice({
  ...
  reducers: {
    addCounter(state, { payload: { initialValue } }: PayloadAction<{ initialValue: number }>) {
    ...
    updateBy(state, { payload: { id, delta } }: PayloadAction<{ id: string; delta: number }>) {
    ...
    updateByPeriodically(
      ...
      if (typeof previousValue === 'number' && Number.isFinite(intervalMs) && intervalMs > 0 && !previousIntervalMs) {
        counterEntity.updateOne(state.counters, {
          id,
          changes: { intervalMs },
        })
...
export const counterSelectors = counterEntity.getSelectors<CounterSlice>((state) => state[counterSlice.name].counters)
